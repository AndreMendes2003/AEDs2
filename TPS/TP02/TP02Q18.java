...
class TP02Q18 {
    ...
    public static void partialQuickSort(Personagem vetor[], int esq, int dir) {
        int pivo = esq;
        int i = esq, j = dir, k = 10;
        while (i <= j) {
            while (vetor[i].getHouse().compareTo(vetor[pivo].getHouse()) < 0 ||
                    (vetor[i].getHouse().compareTo(vetor[pivo].getHouse()) == 0
                            && vetor[i].getName().compareTo(vetor[pivo].getName()) < 0)) {
                i++;
            }
            while (vetor[j].getHouse().compareTo(vetor[pivo].getHouse()) > 0 ||
                    (vetor[j].getHouse().compareTo(vetor[pivo].getHouse()) == 0
                            && vetor[j].getName().compareTo(vetor[pivo].getName()) > 0)) {
                j--;
            }
            if (i <= j) {
                swap(vetor, i, j);
        ...
        if (esq < j)
            partialQuickSort(vetor, esq, j);
        if (i < k && dir > i)
            partialQuickSort(vetor, i, dir);
    }

    public static void swap(Personagem vetor[], int a, int b) {
        Personagem tmp = vetor[a];
        vetor[a] = vetor[b];
        vetor[b] = tmp;
    }
}
