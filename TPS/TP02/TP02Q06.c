...
void selectionSortRecursivo(Personagem p[], int i, int tamVetor)
{
    ...
    {
        comp++;
        if (strcmp(p[j].name, p[menor].name) < 0)
        {
            menor = j;
        }
    ...
    // chamada recursiva
   selectionSortRecursivo(p, i + 1, tamVetor);
}
...
int main()
{
    ...
    selectionSortRecursivo(selectedPersonagens, 0, n);
    ...
    FILE *log;
    if ((log = fopen("matricula_selecaoRecursiva.txt", "w")) != NULL)
    {
