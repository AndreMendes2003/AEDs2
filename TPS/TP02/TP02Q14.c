/*
Repita a questão de Ordenação por Seleção, contudo, usando o algoritmo Radixsort, fazendo com que a
chave de pesquisa seja o atributo id. O nome do arquivo de log será matrícula_radixsort.txt.
*/
...
void Sort(Personagem personagens[], int i, int tamVetor)
{
    if (i >= tamVetor - 1)
    {
        return;
    ...
    for (int j = i + 1; j < tamVetor; j++)
    {
        ...
        const char *s1 = personagens[j].id;
        const char *s2 = personagens[menor].id;
        while (*s1 && *s1 == *s2)
        {
            s1++;
            s2++;
...
int main()
{
    ...
    FILE *log;
    if ((log = fopen("matricula_radixsort.txt", "w")) != NULL)
    {
