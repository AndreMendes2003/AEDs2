...
fazendo com que a chave de pesquisa seja o atributo yearOfBirth. O nome do arquivo de 
log será matrícula_countingsort.txt.
 */
...
class TP02Q11 {
    ...
    public static void countingSort(Personagem vetor[], int n) {
        ...
        // Agora, o count[i] contém o numero de elemento iguais a i
        for (int i = 0; i < n; count[vetor[i].getYearOfBirth()]++, i++)
            ;
        ...
        // Ordenando
        for (int i = n - 1; i >= 0; ordenado[count[vetor[i].getYearOfBirth()] - 1] = vetor[i], mov++, count[vetor[i]
                .getYearOfBirth()]--, i--)
            ;
        ...
        // Desempate (não faz parte do countingSort)
        for (int i = 1; i < n; i++) {
            int j = i - 1;
            Personagem tmp = vetor[i];
            while (j >= 0 && vetor[j].getYearOfBirth() == tmp.getYearOfBirth()
                    && vetor[j].getName().compareTo(tmp.getName()) > 0) {
                vetor[j + 1] = vetor[j];
                j--;
            }
            vetor[j + 1] = tmp;
    ...
    public static int getMaior(Personagem vetor[], int n) {
        int maior = vetor[0].getYearOfBirth();

        for (int i = 0; i < n; i++) {
            if (maior < vetor[i].getYearOfBirth()) {
