/*
Repita a questão de Ordenação por Seleção, contudo, usando o algoritmo Shellsort,
fazendo com que a chave de pesquisa seja o atributo eyeColour. O nome do arquivo de log será matrícula_shellsort.txt.
*/
#include <stdio.h>
#include <string.h>
#include <stdbool.h>
...
void subinsercaoPorCor(Personagem vetor[], int cor, int h, int n)
{
    for (int i = (h + cor); i < n; i += h)
    {
        Personagem tmp = vetor[i];
        int j = i - h;
        while ((j >= 0) && ((strcmp(tmp.eyeColour, vetor[j].eyeColour) < 0) ||
                            ((strcmp(tmp.eyeColour, vetor[j].eyeColour) == 0) && (strcmp(tmp.name, vetor[j].name) < 0))))
        {
            comp += 2;
            vetor[j + h] = vetor[j];
            mov++;
...
void Shellsort(Personagem vetor[], int n)
{
    ...
    do
    {
        ...
        {
            subinsercaoPorCor(vetor, cor, h, n);
        }
    } while (h != 1);
}
...
int main()
{
    ...
    Shellsort(selectedPersonagens, n);
    ...
    FILE *log;
    if ((log = fopen("matricula_shellsort.txt", "w")) != NULL)
    {
