 ...
 */
 ...
 class TP02Q07 {
     ...
     public static void main(String[] args) throws Exception {
         ...
         // Escrever os resultados no arquivo "matricula_insercao.txt"
         try {
     ...
     public static void insertionSort(Personagem vetor[], int n) {
        for (int i = 1; i < n; i++) {
            ...
            while (j > -1 && (tmp.getDateOfBirth().compareTo(vetor[j].getDateOfBirth()) < 0 || (tmp.getDateOfBirth().compareTo(vetor[j].getDateOfBirth()) == 0 && tmp.getName().compareTo(vetor[j].getName()) < 0))) {
                if (tmp.getDateOfBirth().compareTo(vetor[j].getDateOfBirth()) == 0)
                    comp += 3;
                else
                    comp++;
                vetor[j + 1] = vetor[j];
