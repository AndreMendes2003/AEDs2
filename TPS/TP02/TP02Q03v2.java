 ...
 * A chave primária de pesquisa será o atributo name. A entrada padrão é composta por duas partes onde a primeira 
 * é igual a entrada da primeira questão. As demais linhas correspondem a segunda parte. A segunda parte é composta 
 * por várias linhas. Cada uma possui um elemento que deve ser pesquisado no vetor. A última linha terá a palavra 
 * FIM. A saída padrão será composta por várias linhas contendo as palavras SIM/NAO para indicar se existe cada um 
 * dos elementos pesquisados.

Além disso, crie um arquivo de log na pasta corrente com o nome matrícula_sequencial.txt com uma única linha contendo 
sua matrícula, tempo de execução do seu algoritmo e número de comparações. Todas as informações do arquivo de log 
...
public class TP02Q03v2 {
    public static void main(String args[]) throws Exception {
        ...
        while (true) {
            boolean resp = false;
            String search = MyIO.readLine(); // procura irá verificar os nomes
            if (search.equals("FIM")) {
                break;
            }
            for (int i = 0; i < n; i++) {
                count++;
                if (vetor[i].getName().equals(search)) {
                    resp = true;
                    i = n;
                }
        ...
        // Escrever os resultados no arquivo "matricula_sequencial.txt"
        try {
            FileWriter fw = new FileWriter("matricula_sequencial.txt");
            PrintWriter pw = new PrintWriter(fw);
            pw.print("780371\t" + tempo + " segundos\t" + count + " comparacoes");
            pw.close();
        } catch (IOException e) {
