...
usando o algoritmo Heapsort, fazendo com que a chave de pesquisa seja o atributo hairColour. 
...
class TP02Q09 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Escrever os resultados no arquivo "matricula_heapsort.txt"
        try {
    ...
    public static void heapSort(Personagem vetor[], int n) {
        // Construir heap
        ...
        for (int i = n - 1; i > 0; i--) {
            ...
            Personagem temp = vetor[0];
            vetor[0] = vetor[i];
            vetor[i] = temp;
            mov += 3;

            // Chama heap na heap reduzida
            heap(vetor, i, 0);
        }

        // Trabalhar com os empates a partir de "name"
        for (int i = 1; i < n; i++) {
            Personagem tmp = vetor[i];
            int j = i - 1;
            while (j > -1 && (tmp.getHairColour().compareTo(vetor[j].getHairColour()) == 0
                    && tmp.getName().compareTo(vetor[j].getName()) < 0)) {
                comp++;
                vetor[j + 1] = vetor[j];
                mov++;
                j--;
    ...
    public static void heap(Personagem vetor[], int n, int i) {
        int maior = i; // Inicializa o maior como raiz
        int esq = 2 * i + 1; // Filho esquerdo do nó atual
        int dir = 2 * i + 2; // Filho direito do nó atual

        // Se o filho esquerdo for maior que a raiz
        if (esq < n && vetor[esq].getHairColour().compareTo(vetor[maior].getHairColour()) > 0)
            maior = esq;

        // Se o filho direito for maior que o maior até agora
        if (dir < n && vetor[dir].getHairColour().compareTo(vetor[maior].getHairColour()) > 0)
            maior = dir;
        ...
        if (maior != i) {
            ...
            // Chama heap recursivamente no subárvore afetada
            heap(vetor, n, maior);
        }
    }
}
