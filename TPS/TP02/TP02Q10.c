/*
Repita a questão de Ordenação por Seleção, contudo, usando o algoritmo Quicksort, fazendo com que 
a chave de pesquisa seja o atributo house. O nome do arquivo de log será matrícula_quicksort.txt.
*/
...
int Partition(Personagem p[], int low, int high) {
    Personagem pivot = p[high];
    int i = low - 1;

    for (int j = low; j <= high - 1; j++) {
        if (strcmp(p[j].house, pivot.house) < 0 || 
           (strcmp(p[j].house, pivot.house) == 0 && strcmp(p[j].name, pivot.name) < 0)) {
            i++;
            swap(p, i, j);
            mov += 3;
        }
...
void Quicksort(Personagem p[], int low, int high) {
    if (low < high) {
...
int main()
{
    ...
    Quicksort(selectedPersonagens, 0, n-1);
    ...
    FILE *log;
    if ((log = fopen("matricula_quicksort.txt", "w")) != NULL)
    {
